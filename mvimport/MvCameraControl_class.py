...
# Python 3.8+ restricts DLL search paths on Windows; explicitly register lib/ 
# so dependent DLLs (transport layers, GenTL .cti files) resolve correctly.
_lib_dir = os.path.join(os.path.dirname(__file__), "lib")
if os.name == "nt" and hasattr(os, "add_dll_directory"):
    os.add_dll_directory(_lib_dir)

_dll_path = os.path.join(_lib_dir, "MvCameraControl.dll")
# winmode parameter was introduced in Python 3.8; guard for backward compatibility.
if "winmode" in ctypes.WinDLL.__init__.__code__.co_varnames:
    MvCamCtrldll = WinDLL(_dll_path, winmode=0)
else:
